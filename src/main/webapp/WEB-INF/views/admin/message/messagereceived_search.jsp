         ...
         <div class="spacer_"></div> 
            <ul class="sidebar-nav">
                ...
                <li>
                 ...
                 <li>
                    <a href="admin_messageindividual">1:1 쪽지</a>
                </li>
                <li class="active">
                    <a href="#">받은 쪽지함</a>
                </li>
...
<div class="spacer_20"></div>
 <script>
    ...
    function a(){
	 ...
	 document.location.href="admin_receivedmessagesearch?searchString="+searchString;
...
<h6 style="margin-left:10px;">받은 쪽지  ${count} 개</h6>
    <table border="1" width="600px">
     ...
     <c:if test="${empty receivedlistfromuser}">
      <tr>
         <td colspan=5>받은 쪽지가 없습니다.</td>
      </tr>   
     </c:if>    
     <c:forEach var="dto" items="${receivedlistfromuser}">
        <tr>
        <c:if test="${dto.message_isdeletedbyreceiver ne 'Y'}">
            ...
            <td>
                <a href="admin_messagecontent?message_no=${dto.message_no}&message_read=y">${dto.message_title} 
                </a>
            </td>
            ...
            <td>
              <a href="admin_receivedmessagedelete?message_no=${dto.message_no}">삭제
                </a>
            </td>
...
<div class="w3-center w3-padding-32" align="center">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="admin_receivedmessagesearch?searchString=${searchString}&pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="admin_receivedmessagesearch?searchString=${searchString}&pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="admin_receivedmessagesearch?searchString=${searchString}&pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
