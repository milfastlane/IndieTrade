...
<script>
    ...
    function a(){
	 ...
	 document.location.href="admin_sentmessagesearch?searchString="+searchString;
...
<h6 style="margin-left:10px;">보낸 쪽지  ${count} 개</h6>
    <table border="1" width="600px">
        <tr>
            ...
            <th width="10%">받는이</th>
            <th width="5%">읽음상태</th>
            <th width="15%">발송취소(상대방이 읽지 않은 쪽지만 가능)</th>
            <th width="10%">보낸쪽지삭제(상대방의 쪽지함에선 삭제되지 않습니다)</th>
        </tr>
     <c:if test="${empty sentlistfromadmin}">
      <tr>
         <td colspan=7>보낸 쪽지가 없습니다.</td>
      </tr>   
     </c:if>    
     <c:forEach var="dto" items="${sentlistfromadmin}">
        <tr>
            ...
            <td>
                <a href="admin_messagecontent?message_no=${dto.message_no}&message_read=n">${dto.message_title} 
                </a>
            </td>
            ...
            <c:if test="${dto.message_read=='N' and dto.message_receiverid!='all'}">
             <a href="admin_sentmessagedelete?message_no=${dto.message_no}">취소
                </a>
            </c:if>
            </td>
            <td>
            
             <a href="admin_sentmessagehide?message_no=${dto.message_no}">삭제</a>
...
<div class="w3-center w3-padding-32" align="center">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="admin_messagesent?pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="admin_messagesent?pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="admin_messagesent?pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
