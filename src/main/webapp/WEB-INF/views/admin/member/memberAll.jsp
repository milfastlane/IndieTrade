...
<legend>전체 회원 보기</legend>
        ...
        <select name="searchOption" id="searchOption" style="border-radius: 4px;">
         ...
         <button type="button" name="search" class="btn btn-primary" id="search" onclick="a();">조회</button> 
...
<div class="w3-center w3-padding-32" style="margin-right:400px;">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="admin_memberAll?pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="admin_memberAll?pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="admin_memberAll?pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>  
