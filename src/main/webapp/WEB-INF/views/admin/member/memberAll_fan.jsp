    ...
    <div class="col-sm-3 sidenav">
      <ul class="nav nav-pills nav-stacked">
             ...
             <li><a href="admin_memberAll">전체 회원</a></li>
                  ...
                  <li class="active"><a href="admin_memberAll_fan">팬 회원</a></li>
...
<table class="table" style="width: auto !important">
   ...
   <tbody>
    <c:if test="${empty userAllfan}">
      <tr>
         <td colspan=6>등록된 회원이 없습니다.</td>
      </tr>   
   </c:if>

   <c:forEach var="dto" items="${userAllfan}">
      <tr>
        <td>${dto.user_name}</td>
        <td>${dto.user_id}</td>
...
<div class="w3-center w3-padding-32" style="margin-right:400px;">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="admin_memberAll_fan?pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="admin_memberAll_fan?pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="admin_memberAll_fan?pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
