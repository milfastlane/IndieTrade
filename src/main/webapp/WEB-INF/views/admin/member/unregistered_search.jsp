...
<!-- Form Name -->
<legend>조건에 맞는 탈퇴 회원 보기</legend>
         <input type="text" name="searchString" id="searchString" style="border-radius: 4px;" value="${searchString}">
...
<div class="w3-center w3-padding-32" style="margin-right:400px;">
      <div class="w3-bar">
        ...
        <c:if test="${a eq 'yes'}">
         ...
         <a href="admin_unregisteredsearch?searchString=${searchString}&pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="admin_unregisteredsearch?searchString=${searchString}&pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="admin_unregisteredsearch?searchString=${searchString}&pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>    
