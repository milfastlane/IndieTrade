   ...
   <div class="section group notable" style="margin-right:50px;">
      ...
      <c:choose>
         ...
         <c:otherwise>
            ...
            <h2 style="color: gray">Search Genre</h2>
            <c:forEach items="${searchGenre}" var="search">
               <div class="col-md-2 col-sm-2 text-center" >            
                     
                     		<c:choose>
								<c:when test="${search.musicboard_imgname=='null'}">
									<a href="album_view?no=${search.musicboard_no}">
									<img src="resources/img/album.jpg" style="width: 180px; height: 180px;" onerror="this.src='/indietrade/resources/userproimg/album.jpg'"></a>					
								</c:when>
								<c:otherwise>
								 	<a href="album_view?no=${search.musicboard_no}">
                     				<img src="resources/albumImage/${search.musicboard_imgname}" alt="Pic" class="img-responsive"></a>               
								</c:otherwise>
							</c:choose>
                  <h4><span>${search.musicboard_artistname}</span></h4>
                  <div class="spacer"></div>
                  ${search.musicboard_name}
                  <div class="spacer_"></div>
               </div>   
            </c:forEach>
            </div>
