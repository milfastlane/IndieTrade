...
<div class="contaner">
	...
	<div class="section group notable" style="margin-right:50px;">
		<div class="spacer_"></div>
		<c:choose>
			<c:when test="${empty searchViewAlbum}">
				<tr>
					<h3>
			...
			<c:otherwise>
				...
				<h2 style="color: gray">ALBUM</h2>
				<c:forEach items="${searchViewAlbum}" var="search">
		...
		<div class="align_center">
			<c:if test="${a eq 'yes'}">
				<a href="searchviewall?searchString=${searchString}&searchval=album&pageNum=${startPage - pageBlock}"
					class="paged">&lt;</a>		
			</c:if>
			<c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
				<a href="searchviewall?searchString=${searchString}&searchval=album&pageNum=${i}"
					class="paged paged_on">${i}</a>
			</c:forEach>
			<c:if test="${b eq 'yes'}">
				<a href="searchviewall?searchString=${searchString}&searchval=album&pageNum=${startPage + pageBlock}" class="paged">&gt;</a>
			</c:if>
		</div>
</div>
