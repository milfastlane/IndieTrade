...
<div class="spacer_"></div>  
    <div align="center" style="margin-right:60px;">	 	
        ...
        <input type="text" name="searchString" id="searchString" style="border-radius: 4px;">
...
<div class="w3-center w3-padding-32" align="center">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="qnamain?pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="qnamain?pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="qnamain?pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
