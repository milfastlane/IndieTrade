...
<h6 style="margin-left:10px;">받은 쪽지  ${count} 개</h6>
    <table border="1" width="600px">
     ...
     <c:forEach var="dto" items="${receivedlistfromadmin}">
        <tr>
        	<c:if test="${dto.message_isdeletedbysender ne 'Y' and dto.message_isdeletedbyreceiver ne 'Y'}">
...
<div class="w3-center w3-padding-32" align="center">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="user_messagereceived?pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="user_messagereceived?pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="user_messagereceived?pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
