        ...
        <div id="sidebar-wrapper">
         <div class="spacer_"></div> 
            <ul class="sidebar-nav">
        		...
        		<li>
                    <a href="user_messageindividual">다른 유저에게 1:1 쪽지</a>
                </li>
...
<div class="spacer_20"></div>
 <script>
    ...
    function a(){
	 ...
	 document.location.href="user_receivedmessagesearch?searchString="+searchString;
...
<h6 style="margin-left:10px;">받은 쪽지  ${count} 개</h6>
    <table border="1" width="600px">
     ...
     <c:if test="${empty receivedlistfromadmin}">
      <tr>
         <td colspan=5>받은 쪽지가 없습니다.</td>
      </tr>   
     </c:if>    
     <c:forEach var="dto" items="${receivedlistfromadmin}">
        <tr>
        <c:if test="${dto.message_isdeletedbyreceiver ne 'Y'}">
        
            <td>${dto.message_no}</td>
            
            <!-- 게시글 상세보기 페이지로 이동시 게시글 목록페이지에 있는 검색조건, 키워드, 현재페이지 값을 유지하기 위해 -->
            <td>
                <a href="user_messagecontent?message_no=${dto.message_no}&message_read=y">${dto.message_title} 
                </a>
            </td>
            
            <td>${dto.message_datereceived}</td>
             <td>${dto.message_senderid}</td>
            
            <td>
              <a href="user_receivedmessagedelete?message_no=${dto.message_no}">삭제
                </a>
            </td>
...
<div class="w3-center w3-padding-32" align="center">
      ...
      <c:if test="${a eq 'yes'}">
         <a href="user_receivedmessagesearch?searchString=${searchString}&pageNum=${startPage - pageBlock}" class="w3-bar-item w3-button w3-hover-black">«</a>
      </c:if>
      <c:forEach var="i" step="1" begin="${startPage}" end="${endPage}">
       <a href="user_receivedmessagesearch?searchString=${searchString}&pageNum=${i}" class="w3-bar-item w3-button w3-hover-black">${i}</a>
      </c:forEach>
      <c:if test="${b eq 'yes'}">
      <a href="user_receivedmessagesearch?searchString=${searchString}&pageNum=${startPage + pageBlock}" class="w3-bar-item w3-button w3-hover-black">»</a>
      </c:if>      
